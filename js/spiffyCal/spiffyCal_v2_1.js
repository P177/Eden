...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	// English Wweek 
	//var msDOW       = new makeArray0('Su','Mo','Tu','We','Th','Fr','Sa');
	var msDOW       = new makeArray0('Mo','Tu','We','Th','Fr','Sa','Su');
	...
	this.maxYearChoice=dteToday.getFullYear()+50;
	...
	function makeCalendar(intWhatMonth,intWhatYear,bViewOnly) {
		...
		firstDay = new Date(intWhatYear,intWhatMonth,0);
		...
		for (intLoop=1; intLoop<=msDays[intWhatMonth]; intLoop++, intColumn++) {
			...
			if (intColumn > 4) {isWE=true } else {isWE=false}
