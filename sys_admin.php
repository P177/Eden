...
function Admins(){
	...
	if (CheckPriv("groups_admin_add") != 1 || CheckPriv("groups_admin_edit") != 1) { // Pokud nema admin prava pro pridavani atd. zobrazi se mu ve vypisu adminu jen on sam
		...
		if ($_GET['sa'] == "form"){
			...
			if ($_GET['letter'] != "All"){ $like = "AND admin_nick ".$like2." ".AlphabethSelect(mysql_real_escape_string($_GET['letter']));}
		}
...
function AddCategory(){
	...
	if ($_GET['action'] == "admins_cat_add"){
		if (CheckPriv("groups_cat_add") <> 1) { echo _NOTENOUGHPRIV;ShowCategory();exit;}
	}elseif ($_GET['action'] == "admins_cat_edit"){
		if (CheckPriv("groups_cat_edit") <> 1) { echo _NOTENOUGHPRIV;ShowCategory();exit;}
	} else {
...
function ShowCategory(){
	...
	while ($ar = mysql_fetch_array($res)){
		...
		echo "	<td align=\"left\" valign=\"middle\">";
				if (CheckPriv("groups_cat_edit") == 1){echo "<a href=\"sys_admin.php?action=admins_cat_edit&amp;id=".$ar['admin_category_id']."&amp;project=".$_SESSION['project']."\"><img src=\"images/sys_edit.gif\" height=\"18\" width=\"18\" border=\"0\" alt=\""._CMN_EDIT."\"></a>";}
				if (CheckPriv("groups_cat_del") == 1){echo "<a href=\"sys_admin.php?action=admins_cat_del&amp;id=".$ar['admin_category_id']."&amp;project=".$_SESSION['project']."\"><img src=\"images/sys_del.gif\" height=\"18\" width=\"18\" border=\"0\" alt=\""._CMN_DEL."\"></a>";}
		echo "	</td>";
