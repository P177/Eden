				...
				while($pocet >= $i){
					if ($povoleni[$i] != ""){
						...
						*/
						...
						/*
						ALTER TABLE `---dbprefix---_articles` ADD `article_updatecheck` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT '0'
					   
						$res = mysql_query("SELECT article_id, article_perex, article_text FROM "._DB_ARTICLES." WHERE article_updatecheck = 0") or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
						while($ar = mysql_fetch_array($res)){
							$perex = str_ireplace('rel="magic,cz,mtgcard', 'rel="magic,cz,mtgcard_lite', $perex);
							$text = str_ireplace('rel="magic,cz,mtgcard', 'rel="magic,cz,mtgcard_lite', $text);
							echo $ar['article_id']."<br>";
							mysql_query("UPDATE "._DB_ARTICLES." SET article_perex = '$perex', article_text = '$text', article_updatecheck = 1 WHERE article_id = ".$ar['article_id']) or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
						}	*/
						/*
						ALTER TABLE `---dbprefix---_articles` DROP `article_updatecheck` 
						*/
						...
						/*
						ALTER TABLE `---dbprefix---_news` ADD `news_updatecheck` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT '0'
						
						$res = mysql_query("SELECT news_id, news_text FROM "._DB_NEWS." WHERE news_updatecheck = 0") or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
						while($ar = mysql_fetch_array($res)){
						  	$text = str_ireplace('rel="magic,cz,mtgcard', 'rel="magic,cz,mtgcard_lite', $text);
							echo $ar['news_id']."<br>";
							mysql_query("UPDATE "._DB_NEWS." SET news_text = '$text', news_updatecheck = 1 WHERE news_id = ".$ar['news_id']) or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
						}
