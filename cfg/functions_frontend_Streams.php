...
class Stream {
	...
	public function checkActiveChannel($channel,$stream_url = "justintv"){
		...
		switch (strtolower($stream_url)){
			...
			case "own3d":
				...
				break;
			case "twitchtv":
				$channel = htmlspecialchars($channel, ENT_QUOTES);
				
				$clientId = '78566rkufkxrounq3p6uf5elap87m3w';             // Register your application and get a client ID at http://www.twitch.tv/settings?section=applications
				$json_array = json_decode(file_get_contents('https://api.twitch.tv/kraken/streams/'.strtolower($channel).'?client_id='.$clientId), true);
	...
	public function showChannels(){
		...
		$res = mysql_query("
		SELECT article_id, article_headline, article_perex, article_text, article_lang, article_link, article_category_id, 
