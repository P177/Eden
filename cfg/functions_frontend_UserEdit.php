...
class User {
	 ...
	 * @param string		Mode - full, basic...
	 * @return string
     */
	public function showUserDetails($id, $mode = "full"){
		
		$mode =  EdenHelper::prepareInclude($mode);
		
		$res = mysql_query("
		SELECT * 
		FROM "._DB_ADMINS." AS a 
		JOIN "._DB_ADMINS_CLANS." AS acl ON acl.aid = ".(integer)$id." 
		JOIN "._DB_ADMINS_CONTACTS." AS ac ON ac.aid = ".(integer)$id." 
		JOIN "._DB_ADMINS_CONTACTS_SHOP." AS acs ON acs.aid = ".(integer)$id." 
		JOIN "._DB_ADMINS_GAMES." AS ag ON ag.aid = ".(integer)$id." 
		JOIN "._DB_ADMINS_INFO." AS ai ON ai.aid = ".(integer)$id." 
		JOIN "._DB_ADMINS_HW." AS ahw ON ahw.aid = ".(integer)$id." 
		JOIN "._DB_ADMINS_POKER." AS ap ON ap.aid = ".(integer)$id." 
		LEFT JOIN "._DB_ADMINS_GUIDS." AS aguids ON aguids.aid = ".(integer)$id." 
		WHERE a.admin_id = ".(integer)$id." 
		") or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
		$ar = mysql_fetch_assoc($res);
		
		$res_country = mysql_query("
		SELECT country_shortname 
		FROM "._DB_COUNTRIES." 
		WHERE country_id = ".(integer)$ar['admin_contact_country']." 
		") or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
		$ar_country = mysql_fetch_assoc($res_country);
		
		ob_start();
    	include "templates/tpl.users_show_user_details_".$mode.".php";
...
*		NEW USERS																							
*																											
*		$num 			= pocet zobrazenych useru															
*		$word_lenght 	= pocet zobrazenych znaku ve jmene (pocitano od nuly)								
*																											
***********************************************************************************************************/
function NewUsers($num,$word_lenght){
	
	global $db_admin,$db_admin_contact,$db_country,$db_setup;
	global $url_flags;
	...
	$res = mysql_query("SELECT a.admin_id, a.admin_firstname, a.admin_name, a.admin_nick, ac.admin_contact_country FROM $db_admin AS a JOIN $db_admin_contact AS ac ON a.admin_id = ac.aid WHERE admin_reg_allow=1 OR admin_reg_allow=2 ORDER BY admin_id DESC LIMIT ".(integer)$num) or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
	$res_num = mysql_query("SELECT COUNT(*) FROM $db_admin WHERE admin_reg_allow=1 OR admin_reg_allow=2") or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
	$num = mysql_fetch_array($res_num);
	while($ar = mysql_fetch_array($res)){
		$res2 = mysql_query("SELECT country_shortname FROM $db_country WHERE country_id=".(integer)$ar['admin_contact_country']) or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
		...
		echo "<img src=\"".$url_flags.$ar2['country_shortname'].".gif\" width=\"18\" height=\"12\" alt=\""; if ($ar['admin_contact_country'] == 0){$alt = "No Selected";} else { echo NazevVlajky($ar2['country_shortname'],$_GET['lang']);} echo "\">&nbsp;<strong><span class=\"cw_link\"><a href=\"index.php?action=user_details&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."&amp;user_id=".$ar['admin_id']."\">".ShortText($admin_nickname,$word_lenght)."</a></span></strong><br>";
	}
	echo _OVERALL."<a href=\"index.php?action=users_list&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."\">".$num[0]."</a>";
}
/***********************************************************************************************************
*
*				USER DETAILS
...
function UserDetails($uid){
	...
	global $db_admin,$db_admin_clan,$db_admin_contact,$db_admin_game,$db_admin_hw,$db_country;
	global $url_flags;

	$res = mysql_query("SELECT * FROM $db_admin, $db_admin_clan, $db_admin_contact, $db_admin_game, $db_admin_hw WHERE admin_id=".(integer)$uid." AND $db_admin_clan.aid=".(integer)$uid." AND $db_admin_contact.aid=".(integer)$uid." AND $db_admin_game.aid=".(integer)$uid." AND $db_admin_hw.aid=".(integer)$uid) or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
	$ar = mysql_fetch_array($res);
	include "templates/tpl.user.details.php";
}
/***********************************************************************************************************
*
*				USERS LIST
*
***********************************************************************************************************/
function UsersList($letter,$word){
	
	global $db_admin,$db_country,$db_admin_contact;
	global $url_flags;
	...
	if ($_POST['confirm'] == "TRUE"){
		...
		$res = mysql_query("UPDATE $db_admin SET admin_friends='".mysql_real_escape_string($colon_separated)."' WHERE admin_id=".(integer)$_SESSION['loginid']) or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
	}
	if ($letter == "Other"){$like2 = "REGEXP";} else {$like2 = "LIKE";}
	if ($letter != "All"){ $like = "AND admin_nick ".$like2." ".AlphabethSelect($letter, "admin_nick");}
	$res3 = mysql_query("SELECT admin_friends FROM $db_admin WHERE admin_id=".(integer)$_SESSION['loginid']) or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
	$ar3 = mysql_fetch_array($res3);
	$res = mysql_query("SELECT a.admin_id, a.admin_nick, c.admin_contact_country, c.admin_contact_icq, c.admin_contact_xfire FROM $db_admin AS a JOIN $db_admin_contact AS c ON a.admin_id = c.aid WHERE (admin_reg_allow=1 OR admin_reg_allow=2) $like ORDER BY admin_nick ASC") or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
	echo "<form action=\"index.php?action=users&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."\" method=\"post\">";
	while($ar = mysql_fetch_array($res)){
		$pratele = explode (" ", $ar3['admin_friends']); // Rozdeli se pole $ar[admin] na jednotlive uzivatele
		if (in_array ($ar['admin_id'], $pratele)){$pritel = "TRUE";} else {$pritel = "FALSE";}
		$res2 = mysql_query("SELECT country_shortname FROM $db_country WHERE country_id=".(integer)$ar['admin_contact_country']) or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
		$ar2 = mysql_fetch_array($res2);
		if ($ar['admin_contact_country'] == 0){$shortname = "00";} else {$shortname = $ar2['country_shortname'];}
		include "templates/tpl.users.list.php";
	}
	if ($_SESSION['u_status'] == "user" || $_SESSION['u_status'] == "seller" || $_SESSION['u_status'] == "admin"){
		...
		echo "		<input type=\"submit\" value=\""._PM_ADD_TO_FRIENDS."\" class=\"eden_button\">\n";
		echo "	</form>\n";
		echo "	</td>\n";
		echo "</tr>";
	}
