...
*
*		$pid			- Parent ID
*		$modul			- Modul kde se komentare zobrazi (article, news, poll, download, clanwars, user, decklists)
...
*
***********************************************************************************************************/
function Comments($pid,$modul,$string_wrap=90,$ifr_width=480,$ifr_height=400,$td_width=400,$tbl_width=407, $order = "ASC"){
	...
	if ($order == "DESC"){
		// Nothing happen, just check if order is really ASC or DESC
	} else {
		$order = "ASC";
	}
	...
	ORDER BY comment_id $order") or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
	...
	if ($ar_setup['setup_comm_anonym'] == 1 || AGet($_SESSION,'login') != ""){
		$comment_form .= "<div style=\"clear:both\">";
		$comment_form .="<a name=\"form\"></a>";
		$comment_form .="<form action=\"".$eden_cfg['url_edencms']."eden_save.php\" name=\"formular\" method=\"post\" onsubmit=\"return kontrola(this)\">";
		$comment_form .="<table width=\"".$tbl_width."\" align=\"center\" cellpadding=\"4\" cellspacing=\"0\" border=\"0\" class=\"sloupec\">\n";
		if (AGet($_GET,'cmsg') != ""){
			$comment_form .="<tr>\n";
			$comment_form .="	<td class=\"msg_err\" colspan=\"2\">"; if (AGet($_GET,'cmsg') == "bad_captcha"){$comment_form .=_ERR_BAD_CAPTCHA;} $comment_form .="</td>\n";
			$comment_form .="</tr>";
		}
		/* Pokud neni uzivatel mute - zobrazi se formular */
		if($ar6['admin_reg_allow'] != 2){
			$comment_form .="	<tr>\n";
			$comment_form .="		<td valign=\"top\" colspan=\"2\">".PrepareFromDB($ar_setup['setup_lang_comments_rules']); /* Zobrazi pravidla pro komentare */ $comment_form .="<br><br></td>\n";
			$comment_form .="	</tr>"; 
			$comment_form .="	<tr>\n";
			$comment_form .="		<td>"._GUEST_NAME."</td>\n";
			$comment_form .="		<td>"; if ($ar_setup['setup_comm_autofill'] == 1 && ($_SESSION['u_status'] == "user" || $_SESSION['u_status'] == "seller" || $_SESSION['u_status'] == "admin")){$comment_form .="<strong>".$admin_nickname."</strong><input type=\"hidden\" name=\"name\" value=\"".$admin_nickname."\">"; } else { $comment_form .="<input type=\"text\" name=\"name\" size=\"30\" "; if ($_GET['cmsg'] == "bad_captcha"){$comment_form .="value=\"".$_GET['n']."\" ";} $comment_form .="maxlength=\"30\">"; } $comment_form .="</td>\n";
			$comment_form .="	</tr>\n";
			$comment_form .="	<tr>\n";
			$comment_form .="	<td>"._GUEST_EMAIL."</td>\n";
			$comment_form .="		<td>"; if ($ar_setup['setup_comm_autofill'] == 1 && ($_SESSION['u_status'] == "user" || $_SESSION['u_status'] == "seller" || $_SESSION['u_status'] == "admin")){$comment_form .="<strong>".$ar6['admin_email']."</strong><input type=\"hidden\" name=\"email\" value=\"".$ar6['admin_email']."\">"; } else { $comment_form .="<input type=\"text\" name=\"email\" size=\"30\" "; if ($_GET['cmsg'] == "bad_captcha"){$comment_form .="value=\"".$_GET['e']."\" ";} $comment_form .=" maxlength=\"30\">"; } $comment_form .="</td>\n";
			$comment_form .="	</tr>\n";
			$comment_form .="	<tr>\n";
			$comment_form .="		<td>"._GUEST_PREDMET."<br><br></td>\n";
			$comment_form .="		<td><input type=\"text\" name=\"topic\" size=\"40\" maxlength=\"40\" "; if ($_GET['cmsg'] == "bad_captcha"){$comment_form .="value=\"".$_GET['t']."\" ";} $comment_form .="><br><br></td>\n";
			$comment_form .="	</tr>\n";
			if ($_SESSION['u_status'] == "user" || $_SESSION['u_status'] == "seller" || $_SESSION['u_status'] == "admin"){
				...
				//Captcha
				$comment_form .="	<tr>\n";
				$comment_form .="		<td valign=\"top\">Captcha:<br><br></td>\n";
				$comment_form .="		<td>";
				$eden_captcha = new EdenCaptcha($eden_cfg);
				$comment_form .=$eden_captcha->CaptchaShow();
				$comment_form .="		</td>\n";
				$comment_form .="	</tr>\n";
			}
			$comment_form .="	<tr>\n";
			$comment_form .="		<td colspan=\"2\">"._GUEST_COMMENTS."<br>\n";
							 if ($ar_setup['setup_comm_link'] == 1){ $comment_form .="<img alt=\""._GUEST_ADD_LINK."\" class=\"editor_v\" onmouseover=\"className='editor_o'\" onmouseout=\"className='editor_v'\" id=\"HyperLink\" onclick=\"window.open('./edencms/eden_add_link.php?project=".$_SESSION['project']."&input=comments&action=hyperlink&lang=".$_GET['lang']."&filter=".$_GET['filter']."','','menubar=no,resizable=no,toolbar=no,status=no,width=420,height=200')\" height=\"20\" src=\"".$url_eden_images."editor_hyperlink.gif\" width=\"20\"><br>"; } $comment_form .="\n";
			$comment_form .="			<textarea cols=\"40\" rows=\"7\" id=\"comments\" name=\"comments\"> "; if ($_GET['cmsg'] == "bad_captcha"){$comment_form .=$_GET['n'];} $comment_form .="</textarea><br><br>\n";
			$comment_form .="			<input type=\"submit\" value=\""._CMN_SUBMIT."\" class=\"eden_button\">&nbsp;&nbsp;<input type=\"Reset\" value=\"Reset\" class=\"eden_button_reset\">\n";
			$comment_form .="		</td>\n";
			$comment_form .="	</tr>";
			...
			if ($modul == "decklists"){$stranka = "index.php";}
			if ($modul == "clanwars"){$stranka = "index.php";}
			if ($modul == "user"){$stranka = "index.php";}
			$comment_form .="<input type=\"hidden\" name=\"id\" value=\"".$pid."\">\n";
			$comment_form .="<input type=\"hidden\" name=\"project\" value=\"".$project."\">\n";
			$comment_form .="<input type=\"hidden\" name=\"reg_user_comm\" value=\""; if ($_SESSION['u_status'] == "user" || $_SESSION['u_status'] == "seller" || $_SESSION['u_status'] == "admin"){} else {$comment_form .="0";} $comment_form .="\">\n";
			$comment_form .="<input type=\"hidden\" name=\"modul\" value=\"".$modul."\">"; $comment_form .="\n";
			if ($modul == "download"){
				$comment_form .="<input type=\"hidden\" name=\"odkaz\" value=\"".$eden_cfg['misc_web'].$stranka."?action=files&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."&amp;modul=".$modul."&amp;page_mode=".AGet($_GET,'page_mode')."&amp;did=".$_GET['did']."&amp;project=".$project."&amp;dl_id=".$_GET['dl_id']."&amp;dl_id2=".$_GET['dl_id2']."&amp;dl_id3=".$_GET['dl_id3']."&amp;dl_id4=".$_GET['dl_id4']."&amp;dl_id5=".$_GET['dl_id5']."&amp;dl_id6=".$_GET['dl_id6']."&amp;dld=".$_GET['dld']."&amp;dld2=".$_GET['dld2']."&amp;dld3=".$_GET['dld3']."&amp;dld4=".$_GET['dld4']."&amp;dld5=".$_GET['dld5']."&amp;dld6=".$_GET['dld6']."&amp;stav=".$_GET['stav']."&amp;stav2=".$_GET['stav2']."&amp;stav3=".$_GET['stav3']."&amp;stav4=".$_GET['stav4']."&amp;stav5=".$_GET['stav5']."&amp;stav6=".$_GET['stav6']."\">\n"; 
			} elseif ($modul == "decklists"){
				$comment_form .="<input type=\"hidden\" name=\"odkaz\" value=\"".$eden_cfg['misc_web'].$stranka."?action=decklist_show&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."&amp;did=".$_GET['did']."&amp;project=".$project."\">\n"; 
			} elseif ($modul == "poll_users") {
				$comment_form .="<input type=\"hidden\" name=\"odkaz\" value=\"".$eden_cfg['misc_web'].$stranka."?action=users_polls&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."&amp;asid=".$_GET['asid']."&amp;status=".$_GET['status']."\">\n";
			} elseif ($modul == "player") {
				$comment_form .="<input type=\"hidden\" name=\"odkaz\" value=\"".$eden_cfg['misc_web'].$stranka."?action=player&amp;mode=player_acc&amp;id=".$pid."&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."\">\n";
			} elseif ($modul == "user") {
				$comment_form .="<input type=\"hidden\" name=\"odkaz\" value=\"".$eden_cfg['misc_web'].$stranka."?action=user_details&amp;user_id=".$pid."&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."\">\n";
			} elseif ($modul == "clanwars") {
				$comment_form .="<input type=\"hidden\" name=\"odkaz\" value=\"".$eden_cfg['misc_web'].$stranka."?action=clanwars&amp;id_cw=".AGet($_GET,'id_cw')."&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."&amp;stav=open&amp;cw_game2=".AGet($_GET,'cw_game2')."&page=".AGet($_GET,'page')."#".AGet($_GET,'id_cw')."\">\n";
			} else {
				$comment_form .="<input type=\"hidden\" name=\"odkaz\" value=\"".$eden_cfg['misc_web'].$stranka."?action=".$_GET['action']."&amp;lang=".$_GET['lang']."&amp;filter=".$_GET['filter']."&amp;id=".$pid."&amp;modul=".$modul."&amp;page_mode=".AGet($_GET,'page_mode')."\">"; 
			}
			$comment_form .="<input type=\"hidden\" name=\"mode\" value=\"comments\">\n";
		}
		$comment_form .="</table>\n";
   		$comment_form .="</form>\n";
	} else {
		$comment_form .="<table width=\"".$tbl_width."\" align=\"center\" cellpadding=\"4\" cellspacing=\"0\" border=\"0\" class=\"sloupec\">\n";
		$comment_form .="<tr>\n";
		$comment_form .="	<td colspan=\"2\"\">"._MUST_LOGIN."</td>\n";
		$comment_form .="</tr>\n";
		$comment_form .="</table>\n";
	}
	...
	echo "			<td>\n";
		...
		if ($allow_comments == 1) {
			echo "<table align=\"left\" cellpadding=\"3\" cellspacing=\"0\" border=\"0\">"; /* Zapsani poctu komentářů do logu */
			...
			$i = 1;
			$y = $num;
			echo "	<tr><td><br><h2>"._COMM_COMMENTS."</h2></td></tr>";
			$nasobek = 0;
			
			// Show form above comments
			if ($order == "DESC"){
				echo "</table>";
				echo $comment_form;
				echo "<br><br>";
				echo "<table align=\"left\" cellpadding=\"3\" cellspacing=\"0\" border=\"0\">";
			}
			...
			while ($ar = mysql_fetch_array($vysledek)){
				...
				echo "		<td align=\"left\" class=\"eden_comments\"><a name=\"".$cislo."\"></a>".$cislo."&nbsp;\n"; 
							...
							// New comments
							if (($_SESSION['u_status'] == "admin" || $_SESSION['u_status'] == "user" || $_SESSION['u_status'] == "seller") && $i > $ar7['comments_log_comments'] && $order == "ASC"){
								echo "<span class=\"red\">NEW</span>";
							}
							// New comments
							if (($_SESSION['u_status'] == "admin" || $_SESSION['u_status'] == "user" || $_SESSION['u_status'] == "seller") && $y > $ar7['comments_log_comments'] && $order == "DESC"){
								echo "<span class=\"red\">NEW</span>";
							}
					echo "		</td>\n";
