...
function UsersList($letter,$word){
	...
	if ($letter != "All"){ $like = "AND admin_nick ".$like2." ".AlphabethSelect($letter);}
...
function ShowProfile(){
	...
	$res_profile = mysql_query("SELECT profile_id FROM $db_profiles WHERE profile_allow = 1") or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
	
	$profile = Array();
	while ($ar_profile = mysql_fetch_array($res_profile)){
		$profile[] .= $ar_profile['profile_id'];
	}
	$num = count($profile) - 1; // -1 je prevence proti zobrazeni cisla mius rozsah pole
	$random_num = rand (0,$num);
	$random_profile = $profile[$random_num];
	$res_profile = mysql_query("SELECT p.*, g.clan_games_game, c.country_name, c.country_shortname FROM $db_profiles AS p, $db_clan_games AS g, $db_country AS c WHERE p.profile_id=".(integer)$random_profile." AND g.clan_games_id=p.profile_game_id AND c.country_id=p.profile_country_id") or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
