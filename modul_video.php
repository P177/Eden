...
function ShowMain(){
	...
	if ($_GET['action'] != "video_add"){	
		$res = mysql_query("SELECT * FROM $db_clan_awards WHERE clan_award_id=".(float)$_GET['id']) or die ("<strong>File:</strong> ".__FILE__."<br /><strong>Line:</strong>".__LINE__."<br />".mysql_error());
	...
	echo "	</tr>\n";
	echo "	<tr>\n";
	echo "		<td><img src=\"images/sys_manage.gif\" height=\"18\" width=\"18\" border=\"0\"> <a href=\"modul_clan_awards.php?action=video_add&amp;project=".$_SESSION['project']."\">"._CLAN_AWARD_ADD."</a></td>\n";
	echo "	</tr>\n";
	...
	echo "		<td align=\"left\">\n";
	echo "			<input type=\"text\" name=\"clan_award_name\" maxlength=\"80\" size=\"80\" "; if ($_GET['action'] == "video_edit" || $_GET['action'] == "video_del"){echo "value=\"".$ar['clan_award_name']."\"";} echo ">\n";
	echo "		</td>\n";
	echo "	</tr>\n";
	echo "	<tr>\n";
	...
	echo "		</td>\n";
	echo "		<td align=\"left\">\n";
	echo "			<input type=\"text\" name=\"clan_award_link\" maxlength=\"255\" size=\"80\" "; if ($_GET['action'] == "video_edit" || $_GET['action'] == "video_del"){echo "value=\"".$ar['clan_award_link']."\"";} echo ">\n";
	echo "		</td>\n";
	echo "	</tr>\n";
	echo "	<tr>\n";
	...
	echo "			<input type=\"submit\" value=\""; if ($_GET['action'] == "video_del"){echo _CLAN_AWARD_DEL;} elseif ($_GET['action'] == "clan_award_edit"){echo _CLAN_AWARD_EDIT;} else {echo _CLAN_AWARD_ADD;} echo "\" class=\"eden_button\">\n";
	...
	echo "		<td width=\"857\" colspan=\"7\">";
				if ($_GET['y'] == ""){$_GET['y'] = date("Y");}
 				for ($i=FormatDatetime($ar_setup['setup_basic_date'],"Y");$i<=date("Y");$i++){
					...
					if ($ar_award_num[0] > 0){$clan_award_year .= '<a href="modul_clan_awards.php?action=video_add&amp;y='.$i.'&amp;project='.$_SESSION['project'].'">'.$i.'</a>&nbsp;&nbsp;&nbsp;';}else{$clan_award_year .= $i.'&nbsp;&nbsp;&nbsp;';}
				}
		...
		while ($ar_award = mysql_fetch_array($res_award)){
			...
			echo "	<td width=\"65\" valign=\"top\">"; if (CheckPriv("groups_video_edit") == 1){ echo "<a href=\"modul_clan_awards.php?action=video_edit&amp;id=".$ar_award['clan_award_id']."&amp;y=".$_GET['y']."&amp;project=".$_SESSION['project']."\"><img src=\"images/sys_edit.gif\" height=\"18\" width=\"18\" border=\"0\" alt=\""._CMN_EDIT."\"></a>"; }
					if (CheckPriv("groups_video_del") == 1){ echo "<a href=\"modul_clan_awards.php?action=video_del&amp;id=".$ar_award['clan_award_id']."&amp;y=".$_GET['y']."&amp;project=".$_SESSION['project']."\"><img src=\"images/sys_del.gif\" height=\"18\" width=\"18\" border=\"0\" alt=\""._CMN_DEL."\"></a>"; } echo "</td> \n";
